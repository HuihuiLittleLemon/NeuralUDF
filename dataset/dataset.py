...
def load_K_Rt_from_P(filename, P=None):
    ...
    # out = cv.decomposeProjectionMatrix(P)#Note the camera rotation R from OpenCV is w2c (world-to-camera), but t is c2w. 
...
class Dataset:
    def __init__(self, conf):
        ...
        self.intrinsics_all = [] 
        self.pose_all = []
        # zip() 函数会将传入的可迭代对象（这里是两个列表）中对应位置上的元素打包成一个元组，并返回一个迭代器，每次迭代会返回一个元组，包含来自每个可迭代对象的当前元素。如果传入的可迭代对象的长度不同，zip() 函数会在最短的可迭代对象用尽后停止迭代。
        for scale_mat, world_mat in zip(self.scale_mats_np, self.world_mats_np):
            P = world_mat @ scale_mat
            P = P[:3, :4]
            intrinsics, pose = load_K_Rt_from_P(None, P)#P=K(R|Rt)
        ...
        # 下采样过程中使用双线性插值的主要原因是为了保持图像的平滑性和细节。在下采样过程中，由于将图像尺寸缩小，会丢失一部分像素信息，如果不使用插值方法而直接舍弃像素，可能会导致图像出现马赛克或锯齿状的边缘，且可能会损失一些细微的图像特征。
        self.H, self.W = self.images.shape[1], self.images.shape[2]
        self.image_pixels = self.H * self.W

        object_bbox_min = np.array([-1.2, -1.2, -1.2, 1.0])
        object_bbox_max = np.array([1.2, 1.2, 1.2, 1.0])
    ...
    def gen_random_rays_from_all_camera(self, batch_size):
        """
        Generate random rays at world space from all camera.
        """
        # 这里的写法默认输入图片尺寸相同
        img_rand = torch.randint(low=0, high=self.n_images, size=[batch_size])
        pixels_x_rand = torch.randint(low=0, high=self.W, size=[batch_size])
        pixels_y_rand = torch.randint(low=0, high=self.H, size=[batch_size])

        for img_idx in range(self.n_images):
            occupied_mask = np.argwhere(img_rand.to('cpu') == img_idx).squeeze(dim=0)
            pixels_x = pixels_x_rand[occupied_mask]
            pixels_y = pixels_y_rand[occupied_mask]
            # normalized ndc uv coordinates, (-1, 1)
            ndc_u = 2 * pixels_x / (self.W - 1) - 1
            ndc_v = 2 * pixels_y / (self.H - 1) - 1
            rays_ndc_uv = torch.stack([ndc_u, ndc_v], dim=-1).view(-1, 2).float()
            ...
            if img_idx == 0:
                ...
                p_combi = p
                mask_combi = mask
                rays_ndc_uv_combi = rays_ndc_uv
            else:
                rays_combi = torch.cat([rays_combi,rays], dim=0)
                p_combi = torch.cat([p_combi,p], dim=0)
                mask_combi = torch.cat([mask_combi,mask], dim=0)
                rays_ndc_uv_combi = torch.cat([rays_ndc_uv_combi,rays_ndc_uv], dim=0)

        sample = {
            'rays': rays_combi,
            'rays_ndc_uv': rays_ndc_uv_combi.cuda(),
            'rays_norm_XYZ_cam': p_combi.cuda(),  # - XYZ_cam, before multiply depth,
            'rays_patch_color': None,
            'rays_patch_mask': None
        }
