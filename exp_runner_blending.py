...
class Runner:
    def __init__(self, conf_path, mode='train', case='CASE_NAME', model_type='', is_continue=False, args=None):
        ...
        self.anneal_end = self.conf.get_int('train.anneal_end', default=0)
        # don't train the udf network in the early steps
        self.fix_geo_end = self.conf.get_int('train.fix_geo_end', default=500)
        ...
        self.normal_weight = self.conf.get_float('train.normal_weight', default=0.0)
    ...
    def regularization_weights_schedule(self):
        ...
        if self.iter_step > end2:
            sparse_weight = self.sparse_weight # *np.clip((self.iter_step - end2) / end2, 0.0, 1.0)
            # self.sparse_scale_factor = 25000 * np.clip((self.iter_step - end2) / end2, 0.0, 1.0)#训练一开始使用这一项会导致loss无法收敛
    ...
    def adjust_color_loss_weights(self):
        ...
        color_base_weight = self.color_base_weight
        color_weight = self.color_weight
        color_pixel_weight = self.color_pixel_weight
        color_patch_weight = self.color_patch_weight
    ...
    def train_udf(self):
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            sample = self.dataset.gen_random_rays_from_all_camera(self.batch_size)
            data = sample['rays']
            ...
            udf = render_out['udf']
            # udf_loss = torch.exp(-F.l1_loss(udf,torch.zeros_like(udf), reduction='mean'))
            normal_error = render_out['normal_error']

            # Gauss loss
            gauss_error = render_out['gauss_error']
            ...
            loss = color_total_loss + \
                   ...
                   normal_error * self.normal_weight + \
                   gauss_error+ \
            ...
            self.writer.add_scalar('Loss/color_pixel_loss', color_pixel_loss, self.iter_step)
            ...
            self.writer.add_scalar('Loss/gauss', gauss_error, self.iter_step)
            self.writer.add_scalar('Loss/normal', normal_error, self.iter_step)
            ...
            if self.iter_step % self.report_freq == 0:
                ...
                print('iter:{:8>d} loss = {:.4f} '
                      ...
                      'sparse_loss = {:.4f} '
                      'normal_error = {:.4f} '
                      'gauss_error = {:.4f} '.format(self.iter_step, loss, color_total_loss, gradient_error_loss,
            ...
            if self.dataset_name == 'general':
                ...
                if self.iter_step % (self.val_mesh_freq * 2) == 0 and self.vis_ray:
                    for i in range(-self.dataset.H // 4, self.dataset.H // 4, 20):
                        self.visualize_one_ray(img_idx=1, px=self.dataset.W // 2, py=self.dataset.H // 2 + i)#img_idx=33
    ...
    def validate(self, idx=-1, resolution_level=-1, only_color=False):
        ...
        if idx < 0:
            # idx = np.random.randint(self.dataset.n_images) # 随机选取输入图片作为特征图
            idx = np.mod(self.iter_step//self.val_freq, self.dataset.n_images)
    ...
    def extract_udf_mesh(self, world_space=False, resolution=256, dist_threshold_ratio=1.0):
        if self.model_type == 'udf':
            ...
            func = lambda pts: torch.abs(self.sdf_network_fine.sdf(pts))# pts是positions的缩写
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.DEBUG, format=FORMAT) #默认情况下，日志消息会被输出到控制台（即终端窗口），供用户实时查看
    ...
    if args.mode == 'train':
        ...
        for i in range(1):
            ...
            runner.visualize_one_ray(img_idx=1, px=runner.dataset.W // 2 + i, py=runner.dataset.H // 2)#img_idx=48
