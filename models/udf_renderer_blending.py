...
class UDFRendererBlending:
    ...
    def render_core(self,
        ...
        device = z_vals.device #获取张量 z_vals 所在的设备（device）
        ...
        if if_pixel_blending:
            # pts_pixel_mask 是看投影后的像素是否超出[-1, 1]x[-1, 1]
        ...
        # 将所有法向都约束成(1,0,-1)
        # gama = 20
        # ref_normal = torch.Tensor([[1/np.sqrt(2),0,1/np.sqrt(2)],[[1/np.sqrt(2),0,-1/np.sqrt(2)]]]).to('cuda')
        # unit_gradients = gradients / (torch.linalg.norm(gradients, ord=2, dim=-1, keepdim=True) + 1e-5)  # normalize to unit vector
        # normal_dot = unit_gradients@ref_normal.T
        # normal_dot_sq = (normal_dot ** 2).reshape(batch_size, n_samples)
        # normal_error_ = 1 - torch.pow(torch.sum(torch.pow(normal_dot_sq, gama),dim=1),1/gama)
        # normal_error = (near_surface * normal_error_).sum() / (near_surface.sum() + 1e-5)
        ref_normal = torch.Tensor([1/np.sqrt(2),0,1/np.sqrt(2)]).to('cuda')
        unit_gradients = gradients / (torch.linalg.norm(gradients, ord=2, dim=-1, keepdim=True) + 1e-5)  # normalize to unit vector
        normal_dot = unit_gradients@ref_normal.T
        normal_error_ = (1 - normal_dot ** 2).reshape(batch_size, n_samples)
        normal_error = (near_surface * normal_error_).sum() / (near_surface.sum() + 1e-5)

        
        # gauss loss
        # H1, hessians = udf_network.get_gradients_and_H1(pts)
        # low_rank_H1 = calculate_lowrank_H1(H1)
        # loss_fn = torch.nn.MSELoss(reduction='mean')
        # gauss_error = loss_fn(H1, low_rank_H1)
        # gauss_error = (gauss_error).mean()
        gauss_error = 0
    ...
    def render(self, rays_o, rays_d, near, far,
        ...
        gauss_error = ret_fine['gauss_error']
