...
class UDFNetwork(nn.Module):
    ...
    def get_gradients_and_H1(self, x):
        x.requires_grad_(True)
        y = self.udf(x)
        ...
        H1 = torch.cat((hessians, torch.reshape(gradients, (gradients.size()[0], 1, -1))), dim=1)
...
#         dims = [d_hidden + d_out] + [d_hidden for _ in range(n_layers)] + [d_out + blending_cand_views]
...
#         # color_base 再经过一个网络生成 color
#         rendering_input = torch.cat([color_base, x_hidden], dim=-1)
#         x = rendering_input
...
class SingleVarianceNetwork(nn.Module):
    ...
    def forward(self, x):
        return torch.ones([len(x), 1]).to(x.device) * torch.exp(self.variance * 10.0) # len(x)返回的是张量的第一个维度的长度
