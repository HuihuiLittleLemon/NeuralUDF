...
# 这段命令是在Linux环境下使用CUDA_VISIBLE_DEVICES环境变量来指定GPU设备，并运行Python脚本exp_runner_blending.py      
CUDA_VISIBLE_DEVICES=${GPU} python exp_runner_blending.py --conf ./confs/udf_garment_blending.conf \
--case ${CASE} --threshold 0.005 --resolution 256 --vis_ray --reg_weights_schedule --sparse_weight ${SPARSE_WEIGHT} --mode validate_mesh   --is_continue